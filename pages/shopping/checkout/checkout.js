...
Page({
  ...
  getCheckoutInfo: function () {
    ...
    //暂时去掉couponID,如果需要则加入 couponId: that.data.couponId,
    util.request(url, { addressId: that.data.addressId, type: buyType }).then(function (res) {
      if (res.errno === 0) {
  ...
  submitOrder: function () {
    console.log(this.data.addressId,'address id',this.data)
    if (this.data.addressId <= 0) {
      util.showErrorToast('请选择收货地址');
      return false;
    }
    util.request(api.OrderSubmit, { addressId: this.data.addressId, type: this.data.buyType }, 'POST').then(res => {
      if (res.errno === 0) {
